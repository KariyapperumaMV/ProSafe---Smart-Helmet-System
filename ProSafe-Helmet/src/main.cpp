...
const char* COMMAND_API_BASE  = "http://172.20.10.3:5000/api/helmet/command/";
// Dedicated emergency-reset protocol — separate from COMMAND_API_BASE, which
// stays SET_RISK-only. Built at call time as BASE + HELMET_ID + "/emergency/reset"[/ack].
const char* EMERGENCY_RESET_API_BASE = "http://172.20.10.3:5000/api/helmet/";
...
// POST actually succeeds — survives across loop iterations / network drops.
...
void loop() {
    ...
    */
    ...
    // Poll backend for the normal SET_RISK command — kept running even during
    // an emergency so backendRiskState stays current in the background (§18);
    // handleAlerts() is what actually keeps it from affecting the LED.
    ...
    // retried on every loop tick until it succeeds (postJSON's own
    // ensureBackendReachable() cooldown already rate-limits real network
    // attempts when the backend is known to be down, so this doesn't hammer it).
    if (isEmergency && !emergencyPacketSent) {
        emergencyPacketSent = sendEmergencyPacket();
    }

    // While in emergency, poll the dedicated reset endpoint every ~3s.
    if (isEmergency && (millis() - lastEmergencyPollAt >= EMERGENCY_POLL_INTERVAL_MS)) {
        lastEmergencyPollAt = millis();
        pollEmergencyReset();
    }

    // Retry the reset acknowledgement until it succeeds. Independent of
    // isEmergency, which is already false by the time this runs.
    if (ackPending) {
        if (acknowledgeEmergencyReset()) {
            ackPending = false;
        }
    }
...
// ==============================
// Edge-triggered: activateEmergency() fires exactly once per not-emergency
// -> pressed transition, so holding the button down does not re-trigger it.
// Button release is intentionally NOT handled here — per logic.docx, once
// active, emergency mode is only ever cleared by a backend-confirmed reset
// (clearEmergency(), called from pollEmergencyReset()), never locally by
// button release, sensor readings, or a timeout.
void updateEmergencyButton() {
    ...
    if ((now - lastDebounceTime) > EMERGENCY_DEBOUNCE_MS) {
        if (reading != debouncedState) {
            ...
            if (debouncedState == LOW && !isEmergency) {
                activateEmergency();
            }
        }
    }
...
// Sets local emergency state immediately (LED turns red on the very next
// handleAlerts() call, sensor loop away, not a 60s packet wait) and arms the
// packet-send / reset-poll retry loops in loop().
void activateEmergency() {
    isEmergency = true;
    emergencyLatchedAt = millis();
    emergencyPacketSent = false;
    // Poll immediately rather than waiting a full EMERGENCY_POLL_INTERVAL_MS
    // for the first reset check.
    lastEmergencyPollAt = millis() - EMERGENCY_POLL_INTERVAL_MS;
    Serial.println("EMERGENCY ACTIVATED — button pressed");
}

// Called only from pollEmergencyReset() on a genuine {"reset":true} from the
// backend. Does not touch backendRiskState — handleAlerts() falls straight
// back to whatever backend risk was last received, per logic.docx (never
// force SAFE after a reset).
void clearEmergency() {
    isEmergency = false;
    ackPending = true;
    Serial.println("Emergency reset received from backend — resuming normal LED control");
}
...
// ==============================
// The helmet does not decide SAFE/WARNING/CRITICAL itself — that's the
// backend ML pipeline's job (backendRiskState, kept current by
// checkBackendCommand()). Emergency still overrides everything else, same
// as before; that branch is untouched pending the emergency-workflow phase.
// checkConditions() and its warning/critical sensor lists still run every
// loop for Serial diagnostics — they just no longer drive the LED.
void handleAlerts() {
    if (isEmergency) {
...
// doesn't know workerId (backend resolves it from helmetId, same as normal
// packets); gps is included only on a genuine fix, never fabricated.
// Returns success so loop() knows whether to retry.
bool sendEmergencyPacket() {
    ...
    Serial.println("[HTTP] Sending emergency packet");
    return postJSON(EMERGENCY_API_URL, doc);
}

// Polls the dedicated emergency-reset endpoint (separate from the normal
// SET_RISK command endpoint). Only called while isEmergency is true. Any
// failure — unreachable backend, non-200, malformed JSON, missing/false
// "reset" — leaves the helmet exactly as it was: still in emergency, still
// red, tried again on the next poll.
void pollEmergencyReset() {
    ...
    esp_task_wdt_reset();
    HTTPClient http;
    String url = String(EMERGENCY_RESET_API_BASE) + HELMET_ID + "/emergency/reset";
    http.begin(url);
    http.setTimeout(HTTP_TIMEOUT_MS);
    ...
    if (httpCode == 200) {
        ...
        if (!error) {
            bool resetRequested = doc["reset"] | false; // missing/non-bool -> false, stay emergency
            if (resetRequested) {
                clearEmergency();
            }
        }
...
// Called once local emergency has been cleared. Retried from loop() (via
// ackPending) until it succeeds, independent of isEmergency — the ack must
// still go out even though isEmergency is already false by this point.
bool acknowledgeEmergencyReset() {
    StaticJsonDocument<64> doc;
    doc["helmetId"] = HELMET_ID;
    String url = String(EMERGENCY_RESET_API_BASE) + HELMET_ID + "/emergency/reset/ack";
    return postJSON(url.c_str(), doc);
}
...
// ==============================
// Poll backend for the normal risk command. Emergency reset now has its own
// dedicated endpoint/protocol (see pollEmergencyReset()) — this endpoint is
// SET_RISK-only. Firmware keeps polling it even while isEmergency is true
// (§18): backendRiskState stays current in the background, but handleAlerts()
// only applies it when NOT in emergency, so this never affects the LED
...
void checkBackendCommand() {
    ...
    if (httpCode == 200) {
        ...
        DeserializationError error = deserializeJson(doc, response);
        if (!error) {
            // Backend returns { "command": "SET_RISK", "risk": ... } or { "command": null }.
            const char* cmd = doc["command"];
            if (cmd && strcmp(cmd, "SET_RISK") == 0) {
